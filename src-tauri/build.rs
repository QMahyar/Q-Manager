...
fn copy_telethon_worker() {
    ...
    if !source.exists() {
        println!("cargo:warning=Telethon worker not found at {:?}. Run build-telethon script first.", source);
        return;
    }
    ...
    for profile in &["debug", "release"] {
        ...
        match std::fs::copy(&source, &dest) {
            ...
            Err(e) => println!("cargo:warning=Failed to copy Telethon worker to {:?}: {}", dest, e),
        }
    }
    ...
    #[cfg(windows)]
    {
        ...
        if let Ok(entries) = std::fs::read_dir(&build_dir) {
            for entry in entries.flatten() {
                let path = entry.path().join("out").join("x64").join("WebView2Loader.dll");
                if path.exists() {
                    for profile in &["debug", "release"] {
                        ...
                        let deps_dest = target_dir.join(profile).join("deps").join("WebView2Loader.dll");
