...
pub fn target_override_get(
    account_id: i64,
    action_id: i64,
) -> CommandResult<Option<TargetOverride>> {
    let conn = db::get_conn().map_err(error_response)?;
...
pub fn blacklist_list(account_id: i64, action_id: i64) -> CommandResult<Vec<BlacklistEntry>> {
    ...
    let entries = stmt
        .query_map(params![account_id, action_id], |row| {
            Ok(BlacklistEntry {
                id: row.get(0)?,
                account_id: row.get(1)?,
                action_id: row.get(2)?,
...
pub fn blacklist_add(
    ...
    if button_text.len() > 200 {
        return Err(error_response(
            "Button text exceeds maximum length of 200 characters",
        ));
    }
...
pub fn blacklist_remove(entry_id: i64) -> CommandResult<()> {
    let conn = db::get_conn().map_err(error_response)?;
    conn.execute(
        "DELETE FROM target_blacklist WHERE id = ?1",
        params![entry_id],
    )
    .map_err(error_response)?;
    Ok(())
...
pub fn target_pair_add(
    ...
    if target_a.len() > 200 {
        return Err(error_response(
            "Target A exceeds maximum length of 200 characters",
        ));
    }
    if target_b.len() > 200 {
        return Err(error_response(
            "Target B exceeds maximum length of 200 characters",
        ));
    }
    ...
    if current_count >= 50 {
        return Err(error_response(
            "Maximum number of target pairs (50) reached",
        ));
    }
...
pub fn targets_copy(
    ...
    for action_id in &action_ids {
        ...
        let mut stmt = conn
            ...
            .map_err(error_response)?;
        let blacklist: Vec<String> = stmt
            .query_map(params![from_account_id, action_id], |row| row.get(0))
            .map_err(error_response)?
            .filter_map(|r| r.ok())
            .collect();
        ...
        let mut pairs_stmt = conn
            ...
            .map_err(error_response)?;
        let pairs: Vec<(i32, String, String)> = pairs_stmt
            .query_map(params![from_account_id, action_id], |row| {
