...
pub fn group_slots_get(account_id: i64) -> CommandResult<Vec<GroupSlot>> {
    ...
    let slots = stmt.query_map(params![account_id], |row| {
        Ok(GroupSlot {
            id: row.get(0)?,
...
pub async fn account_fetch_groups(account_id: i64) -> CommandResult<Vec<TelegramGroup>> {
    ...
    if !session_dir.exists() {
        return Err(error_response(format!("Session directory not found: {:?}", session_dir)));
    }
    ...
    let response = client.request("list_groups", serde_json::json!({})).map_err(error_response)?;
    if !response.ok {
    ...
    let groups: Vec<TelegramGroup> = serde_json::from_value(
        payload.get("groups").cloned().unwrap_or_else(|| serde_json::json!([]))
    ).map_err(error_response)?;
