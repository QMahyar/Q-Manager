...
use crate::validation::{validate_account_name, validate_phone_number};
...
pub async fn login_start(
...
) -> CommandResult<LoginStartResult> {
    ...
    let (api_id, api_hash) = if let (Some(id), Some(hash)) = (api_id, api_hash) {
        ...
        let id = settings
            .api_id
            .ok_or_else(|| error_response("API ID not configured. Please set it in Settings."))?;
        let hash = settings
            .api_hash
            .ok_or_else(|| error_response("API Hash not configured. Please set it in Settings."))?;
        (id, hash)
    };
...
pub async fn login_get_state(token: String) -> CommandResult<AuthState> {
    ...
    let session = sessions
        .get_mut(&token)
        .ok_or_else(|| error_response("Session not found"))?;
...
pub async fn login_complete(
    ...
    if user_id <= 0 {
        session.shutdown();
        return Err(error_response(
            "Failed to fetch Telegram user info. Please retry login.",
        ));
    }
    ...
    if let Err(err) = std::fs::rename(&temp_dir, &perm_dir) {
        ...
        if let Err(copy_err) = crate::utils::fs::copy_dir_recursive(&temp_dir, &perm_dir) {
            return Err(error_response(format!(
                "Failed to move session directory: {}",
                copy_err
            )));
        }
