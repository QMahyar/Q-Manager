...
pub static LAST_SEEN_DEBOUNCER: Lazy<Debouncer<i64>> = Lazy::new(|| {
    Debouncer::new(Duration::from_secs(30))
});

/// Debouncer for account status events
pub static STATUS_EVENT_DEBOUNCER: Lazy<Debouncer<i64>> = Lazy::new(|| {
...
impl<K: std::hash::Hash + Eq + Clone> Debouncer<K> {
    ...
    pub fn should_execute(&self, key: &K) -> bool {
        let mut map = self.last_execution.lock()
            .expect("Debouncer mutex poisoned - this indicates a panic occurred while holding the lock");
        let now = Instant::now();
    ...
    pub fn check_only(&self, key: &K) -> bool {
        let map = self.last_execution.lock()
            .expect("Debouncer mutex poisoned - this indicates a panic occurred while holding the lock");
        let now = Instant::now();
...
mod tests {
    ...
    fn test_batch_debouncer_overwrites() {
        let debouncer: BatchDebouncer<i64, String> = BatchDebouncer::new(Duration::from_millis(100));
