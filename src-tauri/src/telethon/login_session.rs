...
impl TelethonLoginSession {
    pub fn new(api_id: i64, api_hash: String, session_dir: std::path::PathBuf) -> Result<Self, String> {
        ...
        Ok(Self { api_id, api_hash, session_dir, client, state })
    }
    ...
    pub fn send_phone_number(&mut self, phone: &str) -> Result<AuthState, String> {
        let response = self.client.request("send_phone", serde_json::json!({"phone": phone}))?;
        self.state = parse_state(response)?;
        Ok(self.state.clone())
    }

    pub fn send_code(&mut self, code: &str) -> Result<AuthState, String> {
        let response = self.client.request("send_code", serde_json::json!({"code": code}))?;
        self.state = parse_state(response)?;
        Ok(self.state.clone())
    }

    pub fn send_password(&mut self, password: &str) -> Result<AuthState, String> {
        let response = self.client.request("send_password", serde_json::json!({"password": password}))?;
        self.state = parse_state(response)?;
        Ok(self.state.clone())
    }
