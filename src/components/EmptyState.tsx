...
export function NoSelection({ itemType = "item" }: { itemType?: string }) {
  const article = "aeiouAEIOU".includes(itemType[0] ?? "") ? "an" : "a";
  return (
    <EmptyState
      icon={<IconSettings className="h-8 w-8 text-muted-foreground" />}
      title={`Select ${article} ${itemType}`}
      description={`Choose ${article} ${itemType} from the list to view or edit its settings.`}
    />
  );
}
